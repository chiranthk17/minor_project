...
leaf_model_path = os.environ.get('MODEL_PATH_1', 'plant_model16.ptl')
yolo_model_path = os.environ.get('MODEL_PATH_2', 'best.pt')
...
def predict_yolo(image_path):
    try:
        initial_memory = get_memory_usage()
        start_time = time.time()
        model = YOLO(yolo_model_path)  # Use path from environment variable
